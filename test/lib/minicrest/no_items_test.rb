...
describe Minicrest::NoItems do
  ...
  describe '#matches?' do
    it 'matches when no items match' do
      assert no_items(descends_from(String)).matches?([1, 2, 3])
    end

    it 'does not match when one item matches' do
      refute no_items(descends_from(String)).matches?([1, 'two', 3])
    end

    it 'matches empty array' do
      assert no_items(descends_from(String)).matches?([])
    end
  ...
  describe '#description' do
    it 'describes the item matcher' do
      assert_equal 'no items are an instance of String', no_items(descends_from(String)).description
    end
  end

  describe '#failure_message' do
    it 'shows which item matched' do
      assert_equal no_items(descends_from(String)).failure_message([1, 'two', 3]), <<~MSG.chomp
  ...
  describe '#negated_failure_message' do
    it 'explains that no items matched' do
      assert_equal no_items(descends_from(String)).negated_failure_message([1, 2, 3]), <<~MSG.chomp
  ...
  describe 'with combinators' do
    it 'works with never' do
      assert never(no_items(descends_from(String))).matches?([1, 'two', 3])
      refute never(no_items(descends_from(String))).matches?([1, 2, 3])
    end
  end
end
