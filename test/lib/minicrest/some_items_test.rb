...
describe Minicrest::SomeItems do
  ...
  describe '#matches?' do
    it 'matches when one item matches' do
      assert some_items(is_a(String)).matches?([1, 'two', 3])
    end

    it 'matches when multiple items match' do
      assert some_items(is_a(Integer)).matches?([1, 2, 'three'])
    end

    it 'does not match when no items match' do
      refute some_items(is_a(String)).matches?([1, 2, 3])
    end

    it 'does not match empty array' do
      refute some_items(is_a(Integer)).matches?([])
    end
  ...
  describe '#description' do
    it 'describes the item matcher' do
      assert_equal 'some items are an instance of String', some_items(is_a(String)).description
    end
  end

  describe '#failure_message' do
    it 'explains that no items matched' do
      assert_equal some_items(is_a(String)).failure_message([1, 2, 3]), <<~MSG.chomp
    ...
    it 'explains empty array case' do
      assert_equal some_items(is_a(String)).failure_message([]), <<~MSG.chomp
        expected some items to be an instance of String
        but no items matched
      MSG
    end
  ...
  describe '#negated_failure_message' do
    it 'explains which items matched' do
      assert_equal some_items(is_a(String)).negated_failure_message([1, 'two', 3]), <<~MSG.chomp
  ...
  describe 'with combinators' do
    it 'works with never' do
      assert never(some_items(is_a(String))).matches?([1, 2, 3])
      refute never(some_items(is_a(String))).matches?([1, 'two', 3])
    end
  end
end
