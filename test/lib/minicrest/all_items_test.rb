...
describe Minicrest::AllItems do
  ...
  describe '#matches?' do
    it 'matches when all items match the matcher' do
      assert all_items(descends_from(Integer)).matches?([1, 2, 3])
    end

    it 'does not match when one item fails' do
      refute all_items(descends_from(Integer)).matches?([1, 'two', 3])
    end

    it 'matches empty array' do
      assert all_items(descends_from(Integer)).matches?([])
    end
  ...
  describe '#description' do
    it 'describes the item matcher' do
      assert_equal 'all items are an instance of Integer', all_items(descends_from(Integer)).description
    end
  ...
  describe '#failure_message' do
    it 'shows failing item and index' do
      assert_equal all_items(descends_from(Integer)).failure_message([1, 'two', 3]), <<~MSG.chomp
    ...
    it 'shows first failing item when multiple fail' do
      result = all_items(descends_from(Integer)).failure_message(%w[one two three])
  ...
  describe '#negated_failure_message' do
    it 'explains unexpected match' do
      assert_equal all_items(descends_from(Integer)).negated_failure_message([1, 2, 3]), <<~MSG.chomp
  ...
  describe 'with combinators' do
    it 'works with never' do
      assert never(all_items(descends_from(Integer))).matches?([1, 'two', 3])
      refute never(all_items(descends_from(Integer))).matches?([1, 2, 3])
    end
  end
end
